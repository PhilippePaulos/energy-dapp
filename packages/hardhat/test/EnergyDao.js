...
async function mineBlocks(nbBblocks) {
    for (let i = 0; i < nbBblocks; i++) {
...
async function prepareData() {
    ...
    return { closingTime,  mintAmount, saleAmount, rate, craftsmanPeriod,
  ...
  async function deployFixture() {
    ...
    return { dao, eedToken, sale, governor,  craftsmanPeriod,
...
/*         it("Check added to project", async function () {
            const {dao, craftsman2, eedToken} = await loadFixture(deployFixture);
            await eedToken.connect(craftsman2).approve(dao.address, ethers.utils.parseEther("0.75"));
            await dao.connect(craftsman2).proposeQuotation(0, "devis", IPFS_IMG, 720, 200000);
            const project = await dao.projects(0);
            console.log(project);
            expect(project).to.equal(craftsman2.address);
        }); */
